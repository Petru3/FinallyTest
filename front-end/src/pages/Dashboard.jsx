...
function Dashboard() {
    ...
    const checkAndPostDefaultData = async () => {
        try {
            const response = await fetch('http://localhost:3000/api/data');
            const data = await response.json();
            
            // Check if DefaultData already exists
            const defaultDataTitles = DefaultData.map(d => d.title);
            ...
            console.error('Error checking or posting default data:', error);
        }
    };
